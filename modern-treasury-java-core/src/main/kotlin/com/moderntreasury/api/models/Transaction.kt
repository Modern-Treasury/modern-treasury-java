...
class Transaction @JsonCreator(mode = JsonCreator.Mode.DISABLED) private constructor(
    ...
    class Builder internal constructor() {
        ...
        /** Alias for calling [Builder.asOfTimezone] with `asOfTimezone.orElse(null)`. */
        ...
        fun vendorCodeType(vendorCodeType: Optional<VendorCodeType>) = vendorCodeType(vendorCodeType.getOrNull())
        ...
        fun vendorCustomerId(vendorCustomerId: Optional<String>) = vendorCustomerId(vendorCustomerId.getOrNull())
        ...
        fun vendorDescription(vendorDescription: Optional<String>) = vendorDescription(vendorDescription.getOrNull())
    ...
    fun validate(): Transaction =
        apply {
            ...
            updatedAt()
            vendorCode()
            vendorCodeType().ifPresent { it.validate() }
            vendorCustomerId()
            vendorId()
     ...
     */
    @JvmSynthetic
    internal fun validity(): Int = (if (id.asKnown().isPresent) 1 else 0) + (if (amount.asKnown().isPresent) 1 else 0) + (if (amountString.asKnown().isPresent) 1 else 0) + (if (asOfDate.asKnown().isPresent) 1 else 0) + (if (asOfTime.asKnown().isPresent) 1 else 0) + (if (asOfTimezone.asKnown().isPresent) 1 else 0) + (if (createdAt.asKnown().isPresent) 1 else 0) + (currency.asKnown().getOrNull()?.validity() ?: 0) + (customIdentifiers.asKnown().getOrNull()?.validity() ?: 0) + (if (direction.asKnown().isPresent) 1 else 0) + (if (discardedAt.asKnown().isPresent) 1 else 0) + (foreignExchangeRate.asKnown().getOrNull()?.validity() ?: 0) + (if (internalAccountId.asKnown().isPresent) 1 else 0) + (if (liveMode.asKnown().isPresent) 1 else 0) + (metadata.asKnown().getOrNull()?.validity() ?: 0) + (if (object_.asKnown().isPresent) 1 else 0) + (if (posted.asKnown().isPresent) 1 else 0) + (if (reconciled.asKnown().isPresent) 1 else 0) + (type.asKnown().getOrNull()?.validity() ?: 0) + (if (updatedAt.asKnown().isPresent) 1 else 0) + (if (vendorCode.asKnown().isPresent) 1 else 0) + (vendorCodeType.asKnown().getOrNull()?.validity() ?: 0) + (if (vendorCustomerId.asKnown().isPresent) 1 else 0) + (if (vendorId.asKnown().isPresent) 1 else 0) + (details.asKnown().getOrNull()?.validity() ?: 0) + (if (vendorDescription.asKnown().isPresent) 1 else 0)
    ...
    class VendorCodeType @JsonCreator private constructor(
        ...
        companion object {
            ...
            @JvmField val FAKE_VENDOR = of("fake_vendor")
            ...
            @JvmField val MODERN_TREASURY = of("modern_treasury")
            ...
            @JvmField val WESTERN_ALLIANCE = of("western_alliance")
