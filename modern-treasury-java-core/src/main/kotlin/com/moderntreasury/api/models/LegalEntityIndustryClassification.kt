...
class LegalEntityIndustryClassification @JsonCreator(mode = JsonCreator.Mode.DISABLED) private constructor(
     ...
     */
    @JvmSynthetic
    internal fun validity(): Int = (if (id.asKnown().isPresent) 1 else 0) + (classificationCodes.asKnown().getOrNull()?.size ?: 0) + (classificationType.asKnown().getOrNull()?.validity() ?: 0) + (if (createdAt.asKnown().isPresent) 1 else 0) + (if (discardedAt.asKnown().isPresent) 1 else 0) + (if (liveMode.asKnown().isPresent) 1 else 0) + (if (object_.asKnown().isPresent) 1 else 0) + (if (updatedAt.asKnown().isPresent) 1 else 0)
    ...
    class ClassificationType @JsonCreator private constructor(
        ...
        companion object {
            ...
            @JvmField val ANZSIC = of("anzsic")

            @JvmField val BICS = of("bics")

            @JvmField val GICS = of("gics")

            @JvmField val HSICS = of("hsics")

            @JvmField val ICB = of("icb")

            @JvmField val ISIC = of("isic")

            @JvmField val MGECS = of("mgecs")

            @JvmField val NACE = of("nace")

            @JvmField val NAICS = of("naics")

            @JvmField val RBICS = of("rbics")

            @JvmField val SIC = of("sic")

            @JvmField val SNI = of("sni")

            @JvmField val TRBC = of("trbc")

            @JvmField val UKSIC = of("uksic")

            @JvmField val UNSPSC = of("unspsc")

            @JvmStatic fun of(value: String) = ClassificationType(JsonField.of(value))
        }
