...
class ForeignExchangeRate @JsonCreator(mode = JsonCreator.Mode.DISABLED) private constructor(
     ...
     */
    @JvmSynthetic
    internal fun validity(): Int = (if (baseAmount.asKnown().isPresent) 1 else 0) + (baseCurrency.asKnown().getOrNull()?.validity() ?: 0) + (if (exponent.asKnown().isPresent) 1 else 0) + (if (rateString.asKnown().isPresent) 1 else 0) + (if (targetAmount.asKnown().isPresent) 1 else 0) + (targetCurrency.asKnown().getOrNull()?.validity() ?: 0) + (if (value.asKnown().isPresent) 1 else 0)
