...
class Counterparty @JsonCreator(mode = JsonCreator.Mode.DISABLED) private constructor(
     ...
     */
    @JvmSynthetic
    internal fun validity(): Int = (if (id.asKnown().isPresent) 1 else 0) + (accounts.asKnown().getOrNull()?.sumOf { it.validity().toInt() } ?: 0) + (if (createdAt.asKnown().isPresent) 1 else 0) + (if (discardedAt.asKnown().isPresent) 1 else 0) + (if (email.asKnown().isPresent) 1 else 0) + (if (externalId.asKnown().isPresent) 1 else 0) + (if (legalEntityId.asKnown().isPresent) 1 else 0) + (if (liveMode.asKnown().isPresent) 1 else 0) + (metadata.asKnown().getOrNull()?.validity() ?: 0) + (if (name.asKnown().isPresent) 1 else 0) + (if (object_.asKnown().isPresent) 1 else 0) + (if (sendRemittanceAdvice.asKnown().isPresent) 1 else 0) + (if (updatedAt.asKnown().isPresent) 1 else 0) + (if (verificationStatus.asKnown().isPresent) 1 else 0)
    ...
    class Account @JsonCreator(mode = JsonCreator.Mode.DISABLED) private constructor(
        ...
        /** @throws ModernTreasuryInvalidDataException if the JSON field has an unexpected type (e.g. if the server responded with an unexpected value). */
        fun verificationSource(): Optional<VerificationSource> = verificationSource.getOptional("verification_source")

        /** @throws ModernTreasuryInvalidDataException if the JSON field has an unexpected type (e.g. if the server responded with an unexpected value). */
        fun verificationStatus(): Optional<VerificationStatus> = verificationStatus.getOptional("verification_status")
        ...
        class Builder internal constructor() {
            ...
            fun verificationSource(verificationSource: Optional<VerificationSource>) = verificationSource(verificationSource.getOrNull())
         ...
         */
        @JvmSynthetic
        internal fun validity(): Int = (if (id.asKnown().isPresent) 1 else 0) + (accountDetails.asKnown().getOrNull()?.sumOf { it.validity().toInt() } ?: 0) + (accountType.asKnown().getOrNull()?.validity() ?: 0) + (contactDetails.asKnown().getOrNull()?.sumOf { it.validity().toInt() } ?: 0) + (if (createdAt.asKnown().isPresent) 1 else 0) + (if (discardedAt.asKnown().isPresent) 1 else 0) + (if (externalId.asKnown().isPresent) 1 else 0) + (if (ledgerAccountId.asKnown().isPresent) 1 else 0) + (if (liveMode.asKnown().isPresent) 1 else 0) + (metadata.asKnown().getOrNull()?.validity() ?: 0) + (if (name.asKnown().isPresent) 1 else 0) + (if (object_.asKnown().isPresent) 1 else 0) + (partyAddress.asKnown().getOrNull()?.validity() ?: 0) + (if (partyName.asKnown().isPresent) 1 else 0) + (partyType.asKnown().getOrNull()?.validity() ?: 0) + (routingDetails.asKnown().getOrNull()?.sumOf { it.validity().toInt() } ?: 0) + (if (updatedAt.asKnown().isPresent) 1 else 0) + (verificationSource.asKnown().getOrNull()?.validity() ?: 0) + (verificationStatus.asKnown().getOrNull()?.validity() ?: 0)
