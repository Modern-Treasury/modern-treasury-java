...
class InternalAccountUpdateParams private constructor(
     ...
     */
    fun contraLedgerAccountId(): Optional<String> = body.contraLedgerAccountId()
    ...
    class InternalAccountUpdateRequest @JsonCreator(mode = JsonCreator.Mode.DISABLED) private constructor(
         ...
         */
        @JvmSynthetic
        internal fun validity(): Int = (if (contraLedgerAccountId.asKnown().isPresent) 1 else 0) + (if (counterpartyId.asKnown().isPresent) 1 else 0) + (if (externalId.asKnown().isPresent) 1 else 0) + (if (ledgerAccountId.asKnown().isPresent) 1 else 0) + (metadata.asKnown().getOrNull()?.validity() ?: 0) + (if (name.asKnown().isPresent) 1 else 0) + (if (parentAccountId.asKnown().isPresent) 1 else 0)
