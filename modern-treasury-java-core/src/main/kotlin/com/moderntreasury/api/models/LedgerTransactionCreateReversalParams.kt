...
class LedgerTransactionCreateReversalParams private constructor(
    ...
    class LedgerTransactionReversalCreateRequest @JsonCreator(mode = JsonCreator.Mode.DISABLED) private constructor(
         ...
         */
        @JvmSynthetic
        internal fun validity(): Int = (if (description.asKnown().isPresent) 1 else 0) + (if (effectiveAt.asKnown().isPresent) 1 else 0) + (if (externalId.asKnown().isPresent) 1 else 0) + (if (ledgerableId.asKnown().isPresent) 1 else 0) + (ledgerableType.asKnown().getOrNull()?.validity() ?: 0) + (metadata.asKnown().getOrNull()?.validity() ?: 0) + (status.asKnown().getOrNull()?.validity() ?: 0)
