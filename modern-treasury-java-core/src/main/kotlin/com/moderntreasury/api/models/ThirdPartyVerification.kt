...
class ThirdPartyVerification @JsonCreator(mode = JsonCreator.Mode.DISABLED) private constructor(
     ...
     */
    @JvmSynthetic
    internal fun validity(): Int = (outcome.asKnown().getOrNull()?.validity() ?: 0) + (vendor.asKnown().getOrNull()?.validity() ?: 0) + (if (vendorVerificationId.asKnown().isPresent) 1 else 0) + (verificationCategory.asKnown().getOrNull()?.validity() ?: 0) + (if (verificationMethod.asKnown().isPresent) 1 else 0) + (if (verificationTime.asKnown().isPresent) 1 else 0) + (if (comment.asKnown().isPresent) 1 else 0)
    ...
    class Vendor @JsonCreator private constructor(
        ...
        companion object {
            ...
            @JvmField val IDOLOGY = of("idology")

            @JvmField val SOCURE = of("socure")
