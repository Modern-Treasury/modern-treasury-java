...
class LedgerAccountStatementCreateResponse @JsonCreator(mode = JsonCreator.Mode.DISABLED) private constructor(
     ...
     */
    @JvmSynthetic
    internal fun validity(): Int = (if (id.asKnown().isPresent) 1 else 0) + (if (createdAt.asKnown().isPresent) 1 else 0) + (if (description.asKnown().isPresent) 1 else 0) + (if (effectiveAtLowerBound.asKnown().isPresent) 1 else 0) + (if (effectiveAtUpperBound.asKnown().isPresent) 1 else 0) + (endingBalance.asKnown().getOrNull()?.validity() ?: 0) + (if (ledgerAccountId.asKnown().isPresent) 1 else 0) + (if (ledgerAccountLockVersion.asKnown().isPresent) 1 else 0) + (ledgerAccountNormalBalance.asKnown().getOrNull()?.validity() ?: 0) + (if (ledgerId.asKnown().isPresent) 1 else 0) + (if (liveMode.asKnown().isPresent) 1 else 0) + (metadata.asKnown().getOrNull()?.validity() ?: 0) + (if (object_.asKnown().isPresent) 1 else 0) + (startingBalance.asKnown().getOrNull()?.validity() ?: 0) + (if (updatedAt.asKnown().isPresent) 1 else 0)
