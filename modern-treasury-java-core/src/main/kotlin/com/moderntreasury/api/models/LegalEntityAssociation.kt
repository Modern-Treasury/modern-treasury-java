...
class LegalEntityAssociation @JsonCreator(mode = JsonCreator.Mode.DISABLED) private constructor(
     ...
     * @throws ModernTreasuryInvalidDataException if the JSON field has an unexpected type (e.g. if the server responded with an unexpected value).
     */
    fun ownershipPercentage(): Optional<Long> = ownershipPercentage.getOptional("ownership_percentage")
    ...
    class Builder internal constructor() {
        ...
        /** Alias for calling [Builder.ownershipPercentage] with `ownershipPercentage.orElse(null)`. */
        fun ownershipPercentage(ownershipPercentage: Optional<Long>) = ownershipPercentage(ownershipPercentage.getOrNull())
     ...
     */
    @JvmSynthetic
    internal fun validity(): Int = (if (id.asKnown().isPresent) 1 else 0) + (childLegalEntity.asKnown().getOrNull()?.validity() ?: 0) + (if (createdAt.asKnown().isPresent) 1 else 0) + (if (discardedAt.asKnown().isPresent) 1 else 0) + (if (liveMode.asKnown().isPresent) 1 else 0) + (if (object_.asKnown().isPresent) 1 else 0) + (if (ownershipPercentage.asKnown().isPresent) 1 else 0) + (if (parentLegalEntityId.asKnown().isPresent) 1 else 0) + (relationshipTypes.asKnown().getOrNull()?.sumOf { it.validity().toInt() } ?: 0) + (if (title.asKnown().isPresent) 1 else 0) + (if (updatedAt.asKnown().isPresent) 1 else 0)
