...
class BalanceReportCreateParams private constructor(
    ...
    class BalanceReportCreateRequest @JsonCreator(mode = JsonCreator.Mode.DISABLED) private constructor(
         ...
         */
        @JvmSynthetic
        internal fun validity(): Int = (if (asOfDate.asKnown().isPresent) 1 else 0) + (if (asOfTime.asKnown().isPresent) 1 else 0) + (balanceReportType.asKnown().getOrNull()?.validity() ?: 0) + (balances.asKnown().getOrNull()?.sumOf { it.validity().toInt() } ?: 0)
    ...
    class BalanceCreateRequest @JsonCreator(mode = JsonCreator.Mode.DISABLED) private constructor(
         ...
         */
        @JvmSynthetic
        internal fun validity(): Int = (balanceType.asKnown().getOrNull()?.validity() ?: 0) + (if (vendorCode.asKnown().isPresent) 1 else 0) + (if (vendorCodeType.asKnown().isPresent) 1 else 0) + (if (amount.asKnown().isPresent) 1 else 0) + (if (amountString.asKnown().isPresent) 1 else 0)
