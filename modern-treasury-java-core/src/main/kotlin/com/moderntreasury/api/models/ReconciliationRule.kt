...
class ReconciliationRule @JsonCreator(mode = JsonCreator.Mode.DISABLED) private constructor(
     ...
     * @throws ModernTreasuryInvalidDataException if the JSON field has an unexpected type (e.g. if the server responded with an unexpected value).
     */
    fun customIdentifiers(): Optional<CustomIdentifiers> = customIdentifiers.getOptional("custom_identifiers")
    ...
    class Builder internal constructor() {
        ...
        fun customIdentifiers(customIdentifiers: Optional<CustomIdentifiers>) = customIdentifiers(customIdentifiers.getOrNull())
     ...
     */
    @JvmSynthetic
    internal fun validity(): Int = (if (amountLowerBound.asKnown().isPresent) 1 else 0) + (if (amountUpperBound.asKnown().isPresent) 1 else 0) + (direction.asKnown().getOrNull()?.validity() ?: 0) + (if (internalAccountId.asKnown().isPresent) 1 else 0) + (if (counterpartyId.asKnown().isPresent) 1 else 0) + (currency.asKnown().getOrNull()?.validity() ?: 0) + (customIdentifiers.asKnown().getOrNull()?.validity() ?: 0) + (if (dateLowerBound.asKnown().isPresent) 1 else 0) + (if (dateUpperBound.asKnown().isPresent) 1 else 0) + (type.asKnown().getOrNull()?.validity() ?: 0)
