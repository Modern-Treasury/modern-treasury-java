...
class InvoiceLineItemUpdateParams private constructor(
    ...
    class InvoiceLineItemUpdateRequest @JsonCreator(mode = JsonCreator.Mode.DISABLED) private constructor(
         ...
         * @throws ModernTreasuryInvalidDataException if the JSON field has an unexpected type (e.g. if the server responded with an unexpected value).
         */
        fun unitAmount(): Optional<Long> = unitAmount.getOptional("unit_amount")
         ...
         */
        @JvmSynthetic
        internal fun validity(): Int = (if (description.asKnown().isPresent) 1 else 0) + (if (direction.asKnown().isPresent) 1 else 0) + (metadata.asKnown().getOrNull()?.validity() ?: 0) + (if (name.asKnown().isPresent) 1 else 0) + (if (quantity.asKnown().isPresent) 1 else 0) + (if (unitAmount.asKnown().isPresent) 1 else 0) + (if (unitAmountDecimal.asKnown().isPresent) 1 else 0)
