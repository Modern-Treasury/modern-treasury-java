...
class ContactDetailCreateRequest @JsonCreator(mode = JsonCreator.Mode.DISABLED) private constructor(
    ...
    fun contactIdentifier(): Optional<String> = contactIdentifier.getOptional("contact_identifier")

    /** @throws ModernTreasuryInvalidDataException if the JSON field has an unexpected type (e.g. if the server responded with an unexpected value). */
    fun contactIdentifierType(): Optional<ContactIdentifierType> = contactIdentifierType.getOptional("contact_identifier_type")
     ...
     */
    @JvmSynthetic
    internal fun validity(): Int = (if (contactIdentifier.asKnown().isPresent) 1 else 0) + (contactIdentifierType.asKnown().getOrNull()?.validity() ?: 0)
