...
class VirtualAccountCreateParams private constructor(
    ...
    class VirtualAccountCreateRequest @JsonCreator(mode = JsonCreator.Mode.DISABLED) private constructor(
         ...
         * @throws ModernTreasuryInvalidDataException if the JSON field has an unexpected type (e.g. if the server responded with an unexpected value).
         */
        fun accountDetails(): Optional<List<AccountDetailCreateRequest>> = accountDetails.getOptional("account_details")
         ...
         * @throws ModernTreasuryInvalidDataException if the JSON field has an unexpected type (e.g. if the server responded with an unexpected value).
         */
        fun routingDetails(): Optional<List<RoutingDetailCreateRequest>> = routingDetails.getOptional("routing_details")
         ...
         */
        @JvmSynthetic
        internal fun validity(): Int = (if (internalAccountId.asKnown().isPresent) 1 else 0) + (if (name.asKnown().isPresent) 1 else 0) + (accountDetails.asKnown().getOrNull()?.sumOf { it.validity().toInt() } ?: 0) + (if (counterpartyId.asKnown().isPresent) 1 else 0) + (if (creditLedgerAccountId.asKnown().isPresent) 1 else 0) + (if (debitLedgerAccountId.asKnown().isPresent) 1 else 0) + (if (description.asKnown().isPresent) 1 else 0) + (ledgerAccount.asKnown().getOrNull()?.validity() ?: 0) + (metadata.asKnown().getOrNull()?.validity() ?: 0) + (routingDetails.asKnown().getOrNull()?.sumOf { it.validity().toInt() } ?: 0)
