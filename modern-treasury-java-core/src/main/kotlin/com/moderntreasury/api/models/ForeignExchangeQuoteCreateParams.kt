...
class ForeignExchangeQuoteCreateParams private constructor(
    ...
    class ForeignExchangeQuoteCreateRequest @JsonCreator(mode = JsonCreator.Mode.DISABLED) private constructor(
         ...
         */
        @JvmSynthetic
        internal fun validity(): Int = (if (internalAccountId.asKnown().isPresent) 1 else 0) + (targetCurrency.asKnown().getOrNull()?.validity() ?: 0) + (if (baseAmount.asKnown().isPresent) 1 else 0) + (baseCurrency.asKnown().getOrNull()?.validity() ?: 0) + (if (effectiveAt.asKnown().isPresent) 1 else 0) + (if (targetAmount.asKnown().isPresent) 1 else 0)
