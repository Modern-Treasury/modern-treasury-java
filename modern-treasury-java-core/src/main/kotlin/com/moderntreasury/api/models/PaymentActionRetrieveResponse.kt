...
class PaymentActionRetrieveResponse @JsonCreator(mode = JsonCreator.Mode.DISABLED) private constructor(
     ...
     */
    fun actionableId(): Optional<String> = actionableId.getOptional("actionable_id")
     ...
     * ```java
     * MyClass myObject = paymentActionRetrieveResponse.details().convert(MyClass.class);
     * ```
     */
     ...
     */
    @JvmSynthetic
    internal fun validity(): Int = (if (id.asKnown().isPresent) 1 else 0) + (if (actionableId.asKnown().isPresent) 1 else 0) + (if (actionableType.asKnown().isPresent) 1 else 0) + (if (createdAt.asKnown().isPresent) 1 else 0) + (if (internalAccountId.asKnown().isPresent) 1 else 0) + (if (liveMode.asKnown().isPresent) 1 else 0) + (if (object_.asKnown().isPresent) 1 else 0) + (if (status.asKnown().isPresent) 1 else 0) + (if (type.asKnown().isPresent) 1 else 0) + (if (updatedAt.asKnown().isPresent) 1 else 0)
