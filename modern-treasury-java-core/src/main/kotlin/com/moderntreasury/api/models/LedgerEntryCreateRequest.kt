...
class LedgerEntryCreateRequest @JsonCreator(mode = JsonCreator.Mode.DISABLED) private constructor(
     ...
     */
    @JvmSynthetic
    internal fun validity(): Int = (direction.asKnown().getOrNull()?.validity() ?: 0) + (if (ledgerAccountId.asKnown().isPresent) 1 else 0) + (if (amount.asKnown().isPresent) 1 else 0) + (if (amountString.asKnown().isPresent) 1 else 0) + (availableBalanceAmount.asKnown().getOrNull()?.validity() ?: 0) + (if (effectiveAt.asKnown().isPresent) 1 else 0) + (if (lockVersion.asKnown().isPresent) 1 else 0) + (metadata.asKnown().getOrNull()?.validity() ?: 0) + (pendingBalanceAmount.asKnown().getOrNull()?.validity() ?: 0) + (postedBalanceAmount.asKnown().getOrNull()?.validity() ?: 0) + (if (showResultingLedgerAccountBalances.asKnown().isPresent) 1 else 0)
