...
class RoutingDetail @JsonCreator(mode = JsonCreator.Mode.DISABLED) private constructor(
    ...
    fun bankAddress(): Optional<Address> = bankAddress.getOptional("bank_address")
     ...
     */
    @JvmSynthetic
    internal fun validity(): Int = (if (id.asKnown().isPresent) 1 else 0) + (bankAddress.asKnown().getOrNull()?.validity() ?: 0) + (if (bankName.asKnown().isPresent) 1 else 0) + (if (createdAt.asKnown().isPresent) 1 else 0) + (if (discardedAt.asKnown().isPresent) 1 else 0) + (if (liveMode.asKnown().isPresent) 1 else 0) + (if (object_.asKnown().isPresent) 1 else 0) + (paymentType.asKnown().getOrNull()?.validity() ?: 0) + (if (routingNumber.asKnown().isPresent) 1 else 0) + (routingNumberType.asKnown().getOrNull()?.validity() ?: 0) + (if (updatedAt.asKnown().isPresent) 1 else 0)
