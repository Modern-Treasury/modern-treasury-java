...
class TransactionLineItem @JsonCreator(mode = JsonCreator.Mode.DISABLED) private constructor(
     ...
     * @throws ModernTreasuryInvalidDataException if the JSON field has an unexpected type (e.g. if the server responded with an unexpected value).
     */
    fun reconciliationGroupId(): Optional<String> = reconciliationGroupId.getOptional("reconciliation_group_id")
     ...
     * @throws ModernTreasuryInvalidDataException if the JSON field has an unexpected type (e.g. if the server responded with an unexpected value).
     */
    fun transactableType(): Optional<TransactableType> = transactableType.getOptional("transactable_type")
    ...
    class Builder internal constructor() {
        ...
        fun expectedPaymentId(expectedPaymentId: Optional<String>) = expectedPaymentId(expectedPaymentId.getOrNull())
        ...
        /** Alias for calling [Builder.reconciliationGroupId] with `reconciliationGroupId.orElse(null)`. */
        fun reconciliationGroupId(reconciliationGroupId: Optional<String>) = reconciliationGroupId(reconciliationGroupId.getOrNull())
        ...
        fun transactableId(transactableId: Optional<String>) = transactableId(transactableId.getOrNull())
        ...
        fun transactableType(transactableType: Optional<TransactableType>) = transactableType(transactableType.getOrNull())
     ...
     */
    @JvmSynthetic
    internal fun validity(): Int = (if (id.asKnown().isPresent) 1 else 0) + (if (amount.asKnown().isPresent) 1 else 0) + (if (counterpartyId.asKnown().isPresent) 1 else 0) + (if (createdAt.asKnown().isPresent) 1 else 0) + (if (description.asKnown().isPresent) 1 else 0) + (if (discardedAt.asKnown().isPresent) 1 else 0) + (if (expectedPaymentId.asKnown().isPresent) 1 else 0) + (if (liveMode.asKnown().isPresent) 1 else 0) + (if (object_.asKnown().isPresent) 1 else 0) + (if (reconcilable.asKnown().isPresent) 1 else 0) + (if (reconciliationGroupId.asKnown().isPresent) 1 else 0) + (if (transactableId.asKnown().isPresent) 1 else 0) + (transactableType.asKnown().getOrNull()?.validity() ?: 0) + (if (transactionId.asKnown().isPresent) 1 else 0) + (type.asKnown().getOrNull()?.validity() ?: 0) + (if (updatedAt.asKnown().isPresent) 1 else 0)
