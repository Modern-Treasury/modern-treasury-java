...
class LegalEntityAssociationInlineCreate @JsonCreator(mode = JsonCreator.Mode.DISABLED) private constructor(
     ...
     */
    fun childLegalEntity(): Optional<ChildLegalEntityCreate> = childLegalEntity.getOptional("child_legal_entity")
     ...
     * @throws ModernTreasuryInvalidDataException if the JSON field has an unexpected type (e.g. if the server responded with an unexpected value).
     */
    fun childLegalEntityId(): Optional<String> = childLegalEntityId.getOptional("child_legal_entity_id")
     ...
     */
    @JvmSynthetic
    internal fun validity(): Int = (relationshipTypes.asKnown().getOrNull()?.sumOf { it.validity().toInt() } ?: 0) + (childLegalEntity.asKnown().getOrNull()?.validity() ?: 0) + (if (childLegalEntityId.asKnown().isPresent) 1 else 0) + (if (ownershipPercentage.asKnown().isPresent) 1 else 0) + (if (title.asKnown().isPresent) 1 else 0)
