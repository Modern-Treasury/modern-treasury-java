...
class LedgerAccountStatementCreateParams private constructor(
    ...
    class LedgerAccountStatementCreateRequest @JsonCreator(mode = JsonCreator.Mode.DISABLED) private constructor(
         ...
         */
        @JvmSynthetic
        internal fun validity(): Int = (if (effectiveAtLowerBound.asKnown().isPresent) 1 else 0) + (if (effectiveAtUpperBound.asKnown().isPresent) 1 else 0) + (if (ledgerAccountId.asKnown().isPresent) 1 else 0) + (if (description.asKnown().isPresent) 1 else 0) + (metadata.asKnown().getOrNull()?.validity() ?: 0)
