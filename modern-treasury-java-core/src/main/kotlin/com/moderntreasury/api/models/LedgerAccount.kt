...
class LedgerAccount @JsonCreator(mode = JsonCreator.Mode.DISABLED) private constructor(
     ...
     */
    @JvmSynthetic
    internal fun validity(): Int = (if (id.asKnown().isPresent) 1 else 0) + (balances.asKnown().getOrNull()?.validity() ?: 0) + (if (createdAt.asKnown().isPresent) 1 else 0) + (if (description.asKnown().isPresent) 1 else 0) + (if (discardedAt.asKnown().isPresent) 1 else 0) + (if (externalId.asKnown().isPresent) 1 else 0) + (if (ledgerId.asKnown().isPresent) 1 else 0) + (if (ledgerableId.asKnown().isPresent) 1 else 0) + (ledgerableType.asKnown().getOrNull()?.validity() ?: 0) + (if (liveMode.asKnown().isPresent) 1 else 0) + (if (lockVersion.asKnown().isPresent) 1 else 0) + (metadata.asKnown().getOrNull()?.validity() ?: 0) + (if (name.asKnown().isPresent) 1 else 0) + (normalBalance.asKnown().getOrNull()?.validity() ?: 0) + (if (object_.asKnown().isPresent) 1 else 0) + (if (updatedAt.asKnown().isPresent) 1 else 0)
    ...
    class LedgerBalancesWithEffectiveAt @JsonCreator(mode = JsonCreator.Mode.DISABLED) private constructor(
         ...
         * @throws ModernTreasuryInvalidDataException if the JSON field has an unexpected type (e.g. if the server responded with an unexpected value).
         */
        fun effectiveAtLowerBound(): Optional<OffsetDateTime> = effectiveAtLowerBound.getOptional("effective_at_lower_bound")
         ...
         */
        @JvmSynthetic
        internal fun validity(): Int = (availableBalance.asKnown().getOrNull()?.validity() ?: 0) + (if (effectiveAtLowerBound.asKnown().isPresent) 1 else 0) + (if (effectiveAtUpperBound.asKnown().isPresent) 1 else 0) + (pendingBalance.asKnown().getOrNull()?.validity() ?: 0) + (postedBalance.asKnown().getOrNull()?.validity() ?: 0)
