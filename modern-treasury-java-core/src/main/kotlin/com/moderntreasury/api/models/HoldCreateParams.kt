...
class HoldCreateParams private constructor(
    ...
    class Body @JsonCreator(mode = JsonCreator.Mode.DISABLED) private constructor(
         ...
         */
        @JvmSynthetic
        internal fun validity(): Int = (status.asKnown().getOrNull()?.validity() ?: 0) + (if (targetId.asKnown().isPresent) 1 else 0) + (targetType.asKnown().getOrNull()?.validity() ?: 0) + (metadata.asKnown().getOrNull()?.validity() ?: 0) + (if (reason.asKnown().isPresent) 1 else 0)
