...
class BalanceReport @JsonCreator(mode = JsonCreator.Mode.DISABLED) private constructor(
     ...
     */
    @JvmSynthetic
    internal fun validity(): Int = (if (id.asKnown().isPresent) 1 else 0) + (if (asOfDate.asKnown().isPresent) 1 else 0) + (if (asOfTime.asKnown().isPresent) 1 else 0) + (balanceReportType.asKnown().getOrNull()?.validity() ?: 0) + (balances.asKnown().getOrNull()?.sumOf { it.validity().toInt() } ?: 0) + (if (createdAt.asKnown().isPresent) 1 else 0) + (if (internalAccountId.asKnown().isPresent) 1 else 0) + (if (liveMode.asKnown().isPresent) 1 else 0) + (if (object_.asKnown().isPresent) 1 else 0) + (if (updatedAt.asKnown().isPresent) 1 else 0)
    ...
    class Balance @JsonCreator(mode = JsonCreator.Mode.DISABLED) private constructor(
        ...
        class Builder internal constructor() {
            ...
            /** Alias for calling [Builder.valueDate] with `valueDate.orElse(null)`. */
            ...
            fun vendorCodeType(vendorCodeType: Optional<String>) = vendorCodeType(vendorCodeType.getOrNull())
         ...
         */
        @JvmSynthetic
        internal fun validity(): Int = (if (id.asKnown().isPresent) 1 else 0) + (if (amount.asKnown().isPresent) 1 else 0) + (if (amountString.asKnown().isPresent) 1 else 0) + (if (asOfDate.asKnown().isPresent) 1 else 0) + (if (asOfTime.asKnown().isPresent) 1 else 0) + (balanceType.asKnown().getOrNull()?.validity() ?: 0) + (if (createdAt.asKnown().isPresent) 1 else 0) + (currency.asKnown().getOrNull()?.validity() ?: 0) + (if (liveMode.asKnown().isPresent) 1 else 0) + (if (object_.asKnown().isPresent) 1 else 0) + (if (updatedAt.asKnown().isPresent) 1 else 0) + (if (valueDate.asKnown().isPresent) 1 else 0) + (if (vendorCode.asKnown().isPresent) 1 else 0) + (if (vendorCodeType.asKnown().isPresent) 1 else 0)
        ...
        class BalanceType @JsonCreator private constructor(
            ...
            companion object {
                ...
                @JvmField val OPENING_AVAILABLE_NEXT_BUSINESS_DAY = of("opening_available_next_business_day")
