...
class VirtualAccountUpdateParams private constructor(
    ...
    class VirtualAccountUpdateRequest @JsonCreator(mode = JsonCreator.Mode.DISABLED) private constructor(
         ...
         */
        @JvmSynthetic
        internal fun validity(): Int = (if (counterpartyId.asKnown().isPresent) 1 else 0) + (if (ledgerAccountId.asKnown().isPresent) 1 else 0) + (metadata.asKnown().getOrNull()?.validity() ?: 0) + (if (name.asKnown().isPresent) 1 else 0)
