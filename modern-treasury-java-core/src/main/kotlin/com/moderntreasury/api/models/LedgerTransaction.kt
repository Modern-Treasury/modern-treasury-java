...
class LedgerTransaction @JsonCreator(mode = JsonCreator.Mode.DISABLED) private constructor(
     ...
     */
    @JvmSynthetic
    internal fun validity(): Int = (if (id.asKnown().isPresent) 1 else 0) + (if (archivedReason.asKnown().isPresent) 1 else 0) + (if (createdAt.asKnown().isPresent) 1 else 0) + (if (description.asKnown().isPresent) 1 else 0) + (if (effectiveAt.asKnown().isPresent) 1 else 0) + (if (effectiveDate.asKnown().isPresent) 1 else 0) + (if (externalId.asKnown().isPresent) 1 else 0) + (ledgerEntries.asKnown().getOrNull()?.sumOf { it.validity().toInt() } ?: 0) + (if (ledgerId.asKnown().isPresent) 1 else 0) + (if (ledgerableId.asKnown().isPresent) 1 else 0) + (ledgerableType.asKnown().getOrNull()?.validity() ?: 0) + (if (liveMode.asKnown().isPresent) 1 else 0) + (metadata.asKnown().getOrNull()?.validity() ?: 0) + (if (object_.asKnown().isPresent) 1 else 0) + (if (partiallyPostsLedgerTransactionId.asKnown().isPresent) 1 else 0) + (if (postedAt.asKnown().isPresent) 1 else 0) + (if (reversedByLedgerTransactionId.asKnown().isPresent) 1 else 0) + (if (reversesLedgerTransactionId.asKnown().isPresent) 1 else 0) + (status.asKnown().getOrNull()?.validity() ?: 0) + (if (updatedAt.asKnown().isPresent) 1 else 0)
