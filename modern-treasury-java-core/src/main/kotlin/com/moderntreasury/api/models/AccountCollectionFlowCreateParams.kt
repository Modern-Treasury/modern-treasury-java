...
class AccountCollectionFlowCreateParams private constructor(
    ...
    fun receivingCountries(): Optional<List<ReceivingCountry>> = body.receivingCountries()
    ...
    class AccountCollectionFlowCreateRequest @JsonCreator(mode = JsonCreator.Mode.DISABLED) private constructor(
         ...
         */
        @JvmSynthetic
        internal fun validity(): Int = (if (counterpartyId.asKnown().isPresent) 1 else 0) + (paymentTypes.asKnown().getOrNull()?.size ?: 0) + (receivingCountries.asKnown().getOrNull()?.sumOf { it.validity().toInt() } ?: 0)
