...
class TransactionCreateParams private constructor(
     ...
     * @throws ModernTreasuryInvalidDataException if the JSON field has an unexpected type (e.g. if the server responded with an unexpected value).
     */
    fun amountString(): Optional<String> = body.amountString()
    ...
    class TransactionCreateRequest @JsonCreator(mode = JsonCreator.Mode.DISABLED) private constructor(
         ...
         */
        @JvmSynthetic
        internal fun validity(): Int = (if (asOfDate.asKnown().isPresent) 1 else 0) + (if (direction.asKnown().isPresent) 1 else 0) + (if (internalAccountId.asKnown().isPresent) 1 else 0) + (if (vendorCode.asKnown().isPresent) 1 else 0) + (if (vendorCodeType.asKnown().isPresent) 1 else 0) + (if (amount.asKnown().isPresent) 1 else 0) + (if (amountString.asKnown().isPresent) 1 else 0) + (metadata.asKnown().getOrNull()?.validity() ?: 0) + (if (posted.asKnown().isPresent) 1 else 0) + (type.asKnown().getOrNull()?.validity() ?: 0) + (if (vendorCustomerId.asKnown().isPresent) 1 else 0) + (if (vendorDescription.asKnown().isPresent) 1 else 0)
