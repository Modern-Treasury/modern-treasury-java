...
class ExternalAccountVerifyParams private constructor(
    ...
    class ExternalAccountVerifyRequest @JsonCreator(mode = JsonCreator.Mode.DISABLED) private constructor(
         ...
         */
        @JvmSynthetic
        internal fun validity(): Int = (if (originatingAccountId.asKnown().isPresent) 1 else 0) + (paymentType.asKnown().getOrNull()?.validity() ?: 0) + (currency.asKnown().getOrNull()?.validity() ?: 0) + (fallbackType.asKnown().getOrNull()?.validity() ?: 0) + (priority.asKnown().getOrNull()?.validity() ?: 0)
