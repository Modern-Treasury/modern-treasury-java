...
class ExternalAccountVerifyResponse private constructor(
    ...
    fun verificationAttempt(): Optional<ExternalAccountVerificationAttempt> = Optional.ofNullable(verificationAttempt)
     ...
     * import java.util.Optional;
     *
     * Optional<String> result = externalAccountVerifyResponse.accept(new ExternalAccountVerifyResponse.Visitor<Optional<String>>() {
     *     @Override
     *     public Optional<String> visitExternalAccount(ExternalAccount externalAccount) {
     *         return Optional.of(externalAccount.toString());
     *     }
     *
     *     // ...
     *
    ...
    class ExternalAccountVerificationAttempt @JsonCreator(mode = JsonCreator.Mode.DISABLED) private constructor(
         ...
         */
        @JvmSynthetic
        internal fun validity(): Int = (if (id.asKnown().isPresent) 1 else 0) + (if (createdAt.asKnown().isPresent) 1 else 0) + (if (externalAccountId.asKnown().isPresent) 1 else 0) + (if (liveMode.asKnown().isPresent) 1 else 0) + (if (object_.asKnown().isPresent) 1 else 0) + (if (originatingAccountId.asKnown().isPresent) 1 else 0) + (paymentType.asKnown().getOrNull()?.validity() ?: 0) + (priority.asKnown().getOrNull()?.validity() ?: 0) + (status.asKnown().getOrNull()?.validity() ?: 0) + (if (updatedAt.asKnown().isPresent) 1 else 0)
