...
class LedgerAccountBalanceMonitor @JsonCreator(mode = JsonCreator.Mode.DISABLED) private constructor(
     ...
     */
    @JvmSynthetic
    internal fun validity(): Int = (if (id.asKnown().isPresent) 1 else 0) + (alertCondition.asKnown().getOrNull()?.validity() ?: 0) + (if (createdAt.asKnown().isPresent) 1 else 0) + (currentLedgerAccountBalanceState.asKnown().getOrNull()?.validity() ?: 0) + (if (description.asKnown().isPresent) 1 else 0) + (if (discardedAt.asKnown().isPresent) 1 else 0) + (if (ledgerAccountId.asKnown().isPresent) 1 else 0) + (if (liveMode.asKnown().isPresent) 1 else 0) + (metadata.asKnown().getOrNull()?.validity() ?: 0) + (if (object_.asKnown().isPresent) 1 else 0) + (if (updatedAt.asKnown().isPresent) 1 else 0)
    ...
    class AlertCondition @JsonCreator(mode = JsonCreator.Mode.DISABLED) private constructor(
         ...
         */
        @JvmSynthetic
        internal fun validity(): Int = (if (field.asKnown().isPresent) 1 else 0) + (if (operator.asKnown().isPresent) 1 else 0) + (if (value.asKnown().isPresent) 1 else 0)
    ...
    class CurrentLedgerAccountBalanceState @JsonCreator(mode = JsonCreator.Mode.DISABLED) private constructor(
         ...
         */
        @JvmSynthetic
        internal fun validity(): Int = (balances.asKnown().getOrNull()?.validity() ?: 0) + (if (ledgerAccountLockVersion.asKnown().isPresent) 1 else 0) + (if (triggered.asKnown().isPresent) 1 else 0)
