...
class LegalEntity @JsonCreator(mode = JsonCreator.Mode.DISABLED) private constructor(
     ...
     * @throws ModernTreasuryInvalidDataException if the JSON field has an unexpected type (e.g. if the server responded with an unexpected value).
     */
    fun legalEntityAssociations(): Optional<List<LegalEntityAssociation>> = legalEntityAssociations.getOptional("legal_entity_associations")
    ...
    class Builder internal constructor() {
        ...
        fun discardedAt(discardedAt: Optional<OffsetDateTime>) = discardedAt(discardedAt.getOrNull())
        ...
        fun legalEntityAssociations(legalEntityAssociations: Optional<List<LegalEntityAssociation>>) = legalEntityAssociations(legalEntityAssociations.getOrNull())
     ...
     */
    @JvmSynthetic
    internal fun validity(): Int = (if (id.asKnown().isPresent) 1 else 0) + (addresses.asKnown().getOrNull()?.sumOf { it.validity().toInt() } ?: 0) + (bankSettings.asKnown().getOrNull()?.validity() ?: 0) + (if (businessDescription.asKnown().isPresent) 1 else 0) + (if (businessName.asKnown().isPresent) 1 else 0) + (if (citizenshipCountry.asKnown().isPresent) 1 else 0) + (if (countryOfIncorporation.asKnown().isPresent) 1 else 0) + (if (createdAt.asKnown().isPresent) 1 else 0) + (if (dateFormed.asKnown().isPresent) 1 else 0) + (if (dateOfBirth.asKnown().isPresent) 1 else 0) + (if (discardedAt.asKnown().isPresent) 1 else 0) + (documents.asKnown().getOrNull()?.sumOf { it.validity().toInt() } ?: 0) + (doingBusinessAsNames.asKnown().getOrNull()?.size ?: 0) + (if (email.asKnown().isPresent) 1 else 0) + (if (expectedActivityVolume.asKnown().isPresent) 1 else 0) + (if (externalId.asKnown().isPresent) 1 else 0) + (if (firstName.asKnown().isPresent) 1 else 0) + (identifications.asKnown().getOrNull()?.sumOf { it.validity().toInt() } ?: 0) + (industryClassifications.asKnown().getOrNull()?.sumOf { it.validity().toInt() } ?: 0) + (if (intendedUse.asKnown().isPresent) 1 else 0) + (if (lastName.asKnown().isPresent) 1 else 0) + (legalEntityType.asKnown().getOrNull()?.validity() ?: 0) + (legalStructure.asKnown().getOrNull()?.validity() ?: 0) + (if (listedExchange.asKnown().isPresent) 1 else 0) + (if (liveMode.asKnown().isPresent) 1 else 0) + (metadata.asKnown().getOrNull()?.validity() ?: 0) + (if (middleName.asKnown().isPresent) 1 else 0) + (if (object_.asKnown().isPresent) 1 else 0) + (operatingJurisdictions.asKnown().getOrNull()?.size ?: 0) + (phoneNumbers.asKnown().getOrNull()?.sumOf { it.validity().toInt() } ?: 0) + (if (politicallyExposedPerson.asKnown().isPresent) 1 else 0) + (if (preferredName.asKnown().isPresent) 1 else 0) + (if (prefix.asKnown().isPresent) 1 else 0) + (primarySocialMediaSites.asKnown().getOrNull()?.size ?: 0) + (regulators.asKnown().getOrNull()?.sumOf { it.validity().toInt() } ?: 0) + (riskRating.asKnown().getOrNull()?.validity() ?: 0) + (if (serviceProviderLegalEntityId.asKnown().isPresent) 1 else 0) + (status.asKnown().getOrNull()?.validity() ?: 0) + (if (suffix.asKnown().isPresent) 1 else 0) + (termsOfUse.asKnown().getOrNull()?.validity() ?: 0) + (thirdPartyVerification.asKnown().getOrNull()?.validity() ?: 0) + (thirdPartyVerifications.asKnown().getOrNull()?.sumOf { it.validity().toInt() } ?: 0) + (if (tickerSymbol.asKnown().isPresent) 1 else 0) + (if (updatedAt.asKnown().isPresent) 1 else 0) + (wealthAndEmploymentDetails.asKnown().getOrNull()?.validity() ?: 0) + (if (website.asKnown().isPresent) 1 else 0) + (legalEntityAssociations.asKnown().getOrNull()?.sumOf { it.validity().toInt() } ?: 0)
    ...
    class LegalEntityAddress @JsonCreator(mode = JsonCreator.Mode.DISABLED) private constructor(
         ...
         */
        @JvmSynthetic
        internal fun validity(): Int = (if (id.asKnown().isPresent) 1 else 0) + (addressTypes.asKnown().getOrNull()?.sumOf { it.validity().toInt() } ?: 0) + (if (country.asKnown().isPresent) 1 else 0) + (if (createdAt.asKnown().isPresent) 1 else 0) + (if (discardedAt.asKnown().isPresent) 1 else 0) + (if (line1.asKnown().isPresent) 1 else 0) + (if (line2.asKnown().isPresent) 1 else 0) + (if (liveMode.asKnown().isPresent) 1 else 0) + (if (locality.asKnown().isPresent) 1 else 0) + (if (object_.asKnown().isPresent) 1 else 0) + (if (postalCode.asKnown().isPresent) 1 else 0) + (if (primary.asKnown().isPresent) 1 else 0) + (if (region.asKnown().isPresent) 1 else 0) + (if (updatedAt.asKnown().isPresent) 1 else 0)
