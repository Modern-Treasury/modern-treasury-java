...
class VirtualAccount @JsonCreator(mode = JsonCreator.Mode.DISABLED) private constructor(
     ...
     * @throws ModernTreasuryInvalidDataException if the JSON field has an unexpected type (e.g. if the server responded with an unexpected value).
     */
    fun creditLedgerAccountId(): Optional<String> = creditLedgerAccountId.getOptional("credit_ledger_account_id")
     ...
     * @throws ModernTreasuryInvalidDataException if the JSON field has an unexpected type (e.g. if the server responded with an unexpected value).
     */
    fun debitLedgerAccountId(): Optional<String> = debitLedgerAccountId.getOptional("debit_ledger_account_id")
    ...
    class Builder internal constructor() {
        ...
        /** Alias for calling [Builder.creditLedgerAccountId] with `creditLedgerAccountId.orElse(null)`. */
        fun creditLedgerAccountId(creditLedgerAccountId: Optional<String>) = creditLedgerAccountId(creditLedgerAccountId.getOrNull())
        ...
        /** Alias for calling [Builder.debitLedgerAccountId] with `debitLedgerAccountId.orElse(null)`. */
        fun debitLedgerAccountId(debitLedgerAccountId: Optional<String>) = debitLedgerAccountId(debitLedgerAccountId.getOrNull())
     ...
     */
    @JvmSynthetic
    internal fun validity(): Int = (if (id.asKnown().isPresent) 1 else 0) + (accountDetails.asKnown().getOrNull()?.sumOf { it.validity().toInt() } ?: 0) + (if (counterpartyId.asKnown().isPresent) 1 else 0) + (if (createdAt.asKnown().isPresent) 1 else 0) + (if (creditLedgerAccountId.asKnown().isPresent) 1 else 0) + (if (debitLedgerAccountId.asKnown().isPresent) 1 else 0) + (if (description.asKnown().isPresent) 1 else 0) + (if (discardedAt.asKnown().isPresent) 1 else 0) + (if (internalAccountId.asKnown().isPresent) 1 else 0) + (if (ledgerAccountId.asKnown().isPresent) 1 else 0) + (if (liveMode.asKnown().isPresent) 1 else 0) + (metadata.asKnown().getOrNull()?.validity() ?: 0) + (if (name.asKnown().isPresent) 1 else 0) + (if (object_.asKnown().isPresent) 1 else 0) + (routingDetails.asKnown().getOrNull()?.sumOf { it.validity().toInt() } ?: 0) + (if (updatedAt.asKnown().isPresent) 1 else 0)
