...
class InternalAccount @JsonCreator(mode = JsonCreator.Mode.DISABLED) private constructor(
     ...
     * @throws ModernTreasuryInvalidDataException if the JSON field has an unexpected type (e.g. if the server responded with an unexpected value).
     */
    fun contraLedgerAccountId(): Optional<String> = contraLedgerAccountId.getOptional("contra_ledger_account_id")
    ...
    class Builder internal constructor() {
        ...
        /** Alias for calling [Builder.contraLedgerAccountId] with `contraLedgerAccountId.orElse(null)`. */
        fun contraLedgerAccountId(contraLedgerAccountId: Optional<String>) = contraLedgerAccountId(contraLedgerAccountId.getOrNull())
        ...
        fun parentAccountId(parentAccountId: Optional<String>) = parentAccountId(parentAccountId.getOrNull())
     ...
     */
    @JvmSynthetic
    internal fun validity(): Int = (if (id.asKnown().isPresent) 1 else 0) + (accountCapabilities.asKnown().getOrNull()?.sumOf { it.validity().toInt() } ?: 0) + (accountDetails.asKnown().getOrNull()?.sumOf { it.validity().toInt() } ?: 0) + (accountType.asKnown().getOrNull()?.validity() ?: 0) + (connection.asKnown().getOrNull()?.validity() ?: 0) + (if (contraLedgerAccountId.asKnown().isPresent) 1 else 0) + (if (counterpartyId.asKnown().isPresent) 1 else 0) + (if (createdAt.asKnown().isPresent) 1 else 0) + (currency.asKnown().getOrNull()?.validity() ?: 0) + (if (debitable.asKnown().isPresent) 1 else 0) + (if (externalId.asKnown().isPresent) 1 else 0) + (if (ledgerAccountId.asKnown().isPresent) 1 else 0) + (if (legalEntityId.asKnown().isPresent) 1 else 0) + (if (liveMode.asKnown().isPresent) 1 else 0) + (metadata.asKnown().getOrNull()?.validity() ?: 0) + (if (name.asKnown().isPresent) 1 else 0) + (if (object_.asKnown().isPresent) 1 else 0) + (if (parentAccountId.asKnown().isPresent) 1 else 0) + (partyAddress.asKnown().getOrNull()?.validity() ?: 0) + (if (partyName.asKnown().isPresent) 1 else 0) + (partyType.asKnown().getOrNull()?.validity() ?: 0) + (routingDetails.asKnown().getOrNull()?.sumOf { it.validity().toInt() } ?: 0) + (status.asKnown().getOrNull()?.validity() ?: 0) + (if (updatedAt.asKnown().isPresent) 1 else 0) + (if (vendorId.asKnown().isPresent) 1 else 0)
    ...
    class AccountCapability @JsonCreator(mode = JsonCreator.Mode.DISABLED) private constructor(
         ...
         */
        @JvmSynthetic
        internal fun validity(): Int = (if (id.asKnown().isPresent) 1 else 0) + (if (createdAt.asKnown().isPresent) 1 else 0) + (direction.asKnown().getOrNull()?.validity() ?: 0) + (if (discardedAt.asKnown().isPresent) 1 else 0) + (if (identifier.asKnown().isPresent) 1 else 0) + (if (liveMode.asKnown().isPresent) 1 else 0) + (if (object_.asKnown().isPresent) 1 else 0) + (paymentType.asKnown().getOrNull()?.validity() ?: 0) + (if (updatedAt.asKnown().isPresent) 1 else 0)
