...
class BulkResult @JsonCreator(mode = JsonCreator.Mode.DISABLED) private constructor(
    ...
    class Builder internal constructor() {
        ...
        /** Alias for calling [Builder.requestParams] with `requestParams.orElse(null)`. */
        fun requestParams(requestParams: Optional<RequestParams>) = requestParams(requestParams.getOrNull())
    ...
    fun validate(): BulkResult =
        apply {
            ...
            object_()
            requestId()
            requestParams().ifPresent { it.validate() }
     ...
     */
    @JvmSynthetic
    internal fun validity(): Int = (if (id.asKnown().isPresent) 1 else 0) + (if (createdAt.asKnown().isPresent) 1 else 0) + (entity.asKnown().getOrNull()?.validity() ?: 0) + (if (entityId.asKnown().isPresent) 1 else 0) + (entityType.asKnown().getOrNull()?.validity() ?: 0) + (if (liveMode.asKnown().isPresent) 1 else 0) + (if (object_.asKnown().isPresent) 1 else 0) + (if (requestId.asKnown().isPresent) 1 else 0) + (requestParams.asKnown().getOrNull()?.validity() ?: 0) + (requestType.asKnown().getOrNull()?.validity() ?: 0) + (status.asKnown().getOrNull()?.validity() ?: 0) + (if (updatedAt.asKnown().isPresent) 1 else 0)
    ...
    class Entity private constructor(
        ...
        fun paymentOrder(): Optional<PaymentOrder> = Optional.ofNullable(paymentOrder)

        fun expectedPayment(): Optional<ExpectedPayment> = Optional.ofNullable(expectedPayment)

        fun ledgerTransaction(): Optional<LedgerTransaction> = Optional.ofNullable(ledgerTransaction)
         ...
         * import java.util.Optional;
         *
         * Optional<String> result = entity.accept(new Entity.Visitor<Optional<String>>() {
         *     @Override
         *     public Optional<String> visitPaymentOrder(PaymentOrder paymentOrder) {
         *         return Optional.of(paymentOrder.toString());
         *     }
         *
         *     // ...
         *
        ...
        class BulkError @JsonCreator(mode = JsonCreator.Mode.DISABLED) private constructor(
             ...
             */
            @JvmSynthetic
            internal fun validity(): Int = (if (id.asKnown().isPresent) 1 else 0) + (if (createdAt.asKnown().isPresent) 1 else 0) + (if (liveMode.asKnown().isPresent) 1 else 0) + (if (object_.asKnown().isPresent) 1 else 0) + (requestErrors.asKnown().getOrNull()?.sumOf { it.validity().toInt() } ?: 0) + (if (updatedAt.asKnown().isPresent) 1 else 0)
            ...
            class RequestError @JsonCreator(mode = JsonCreator.Mode.DISABLED) private constructor(
                 ...
                 */
                @JvmSynthetic
                internal fun validity(): Int = (if (code.asKnown().isPresent) 1 else 0) + (if (message.asKnown().isPresent) 1 else 0) + (if (parameter.asKnown().isPresent) 1 else 0)
