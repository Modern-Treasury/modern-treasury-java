...
class LedgerAccountCategoryCreateParams private constructor(
    ...
    class LedgerAccountCategoryCreateRequest @JsonCreator(mode = JsonCreator.Mode.DISABLED) private constructor(
         ...
         */
        @JvmSynthetic
        internal fun validity(): Int = (if (currency.asKnown().isPresent) 1 else 0) + (if (ledgerId.asKnown().isPresent) 1 else 0) + (if (name.asKnown().isPresent) 1 else 0) + (normalBalance.asKnown().getOrNull()?.validity() ?: 0) + (if (currencyExponent.asKnown().isPresent) 1 else 0) + (if (description.asKnown().isPresent) 1 else 0) + (if (externalId.asKnown().isPresent) 1 else 0) + (ledgerAccountCategoryIds.asKnown().getOrNull()?.size ?: 0) + (metadata.asKnown().getOrNull()?.validity() ?: 0)
