...
class BankSettings @JsonCreator(mode = JsonCreator.Mode.DISABLED) private constructor(
     ...
     */
    fun backupWithholdingPercentage(): Optional<Long> = backupWithholdingPercentage.getOptional("backup_withholding_percentage")
     ...
     * @throws ModernTreasuryInvalidDataException if the JSON field has an unexpected type (e.g. if the server responded with an unexpected value).
     */
    fun enableBackupWithholding(): Optional<Boolean> = enableBackupWithholding.getOptional("enable_backup_withholding")
    ...
    class Builder internal constructor() {
        ...
        /** Alias for calling [Builder.backupWithholdingPercentage] with `backupWithholdingPercentage.orElse(null)`. */
        fun backupWithholdingPercentage(backupWithholdingPercentage: Optional<Long>) = backupWithholdingPercentage(backupWithholdingPercentage.getOrNull())
        ...
        /** Alias for calling [Builder.enableBackupWithholding] with `enableBackupWithholding.orElse(null)`. */
        fun enableBackupWithholding(enableBackupWithholding: Optional<Boolean>) = enableBackupWithholding(enableBackupWithholding.getOrNull())
        ...
        fun privacyOptOut(privacyOptOut: Optional<Boolean>) = privacyOptOut(privacyOptOut.getOrNull())
     ...
     */
    @JvmSynthetic
    internal fun validity(): Int = (if (id.asKnown().isPresent) 1 else 0) + (if (backupWithholdingPercentage.asKnown().isPresent) 1 else 0) + (if (createdAt.asKnown().isPresent) 1 else 0) + (if (discardedAt.asKnown().isPresent) 1 else 0) + (if (enableBackupWithholding.asKnown().isPresent) 1 else 0) + (if (liveMode.asKnown().isPresent) 1 else 0) + (if (object_.asKnown().isPresent) 1 else 0) + (if (privacyOptOut.asKnown().isPresent) 1 else 0) + (if (regulationO.asKnown().isPresent) 1 else 0) + (if (updatedAt.asKnown().isPresent) 1 else 0)
