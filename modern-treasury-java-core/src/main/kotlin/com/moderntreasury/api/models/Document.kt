...
class Document @JsonCreator(mode = JsonCreator.Mode.DISABLED) private constructor(
     ...
     * @throws ModernTreasuryInvalidDataException if the JSON field has an unexpected type (e.g. if the server responded with an unexpected value).
     */
    fun documentableId(): Optional<String> = documentableId.getOptional("documentable_id")
     ...
     * @throws ModernTreasuryInvalidDataException if the JSON field has an unexpected type (e.g. if the server responded with an unexpected value).
     */
    fun documentableType(): Optional<DocumentableType> = documentableType.getOptional("documentable_type")
    ...
    class Builder internal constructor() {
        ...
        /** Alias for calling [Builder.documentType] with `documentType.orElse(null)`. */
        ...
        fun documentableId(documentableId: Optional<String>) = documentableId(documentableId.getOrNull())
        ...
        fun documentableType(documentableType: Optional<DocumentableType>) = documentableType(documentableType.getOrNull())
     ...
     */
    @JvmSynthetic
    internal fun validity(): Int = (if (id.asKnown().isPresent) 1 else 0) + (if (createdAt.asKnown().isPresent) 1 else 0) + (if (discardedAt.asKnown().isPresent) 1 else 0) + (if (documentType.asKnown().isPresent) 1 else 0) + (if (documentableId.asKnown().isPresent) 1 else 0) + (documentableType.asKnown().getOrNull()?.validity() ?: 0) + (file.asKnown().getOrNull()?.validity() ?: 0) + (if (liveMode.asKnown().isPresent) 1 else 0) + (if (object_.asKnown().isPresent) 1 else 0) + (if (source.asKnown().isPresent) 1 else 0) + (if (updatedAt.asKnown().isPresent) 1 else 0)
    ...
    class File @JsonCreator(mode = JsonCreator.Mode.DISABLED) private constructor(
         ...
         */
        @JvmSynthetic
        internal fun validity(): Int = (if (contentType.asKnown().isPresent) 1 else 0) + (if (filename.asKnown().isPresent) 1 else 0) + (if (size.asKnown().isPresent) 1 else 0)
