...
class PaymentFlowCreateParams private constructor(
    ...
    class PaymentFlowCreateRequest @JsonCreator(mode = JsonCreator.Mode.DISABLED) private constructor(
         ...
         */
        @JvmSynthetic
        internal fun validity(): Int = (if (amount.asKnown().isPresent) 1 else 0) + (if (counterpartyId.asKnown().isPresent) 1 else 0) + (if (currency.asKnown().isPresent) 1 else 0) + (direction.asKnown().getOrNull()?.validity() ?: 0) + (if (originatingAccountId.asKnown().isPresent) 1 else 0) + (if (dueDate.asKnown().isPresent) 1 else 0)
