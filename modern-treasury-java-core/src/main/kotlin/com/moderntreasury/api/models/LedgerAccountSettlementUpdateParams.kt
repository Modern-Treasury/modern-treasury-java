...
class LedgerAccountSettlementUpdateParams private constructor(
    ...
    class LedgerAccountSettlementUpdateRequest @JsonCreator(mode = JsonCreator.Mode.DISABLED) private constructor(
         ...
         */
        @JvmSynthetic
        internal fun validity(): Int = (if (description.asKnown().isPresent) 1 else 0) + (metadata.asKnown().getOrNull()?.validity() ?: 0) + (if (skipSettlementLedgerTransaction.asKnown().isPresent) 1 else 0) + (status.asKnown().getOrNull()?.validity() ?: 0)
