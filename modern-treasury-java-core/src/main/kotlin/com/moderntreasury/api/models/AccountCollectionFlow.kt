...
class AccountCollectionFlow @JsonCreator(mode = JsonCreator.Mode.DISABLED) private constructor(
    ...
    /** @throws ModernTreasuryInvalidDataException if the JSON field has an unexpected type (e.g. if the server responded with an unexpected value). */
    fun receivingCountries(): Optional<List<ReceivingCountry>> = receivingCountries.getOptional("receiving_countries")
     ...
     */
    @JvmSynthetic
    internal fun validity(): Int = (if (counterpartyId.asKnown().isPresent) 1 else 0) + (paymentTypes.asKnown().getOrNull()?.sumOf { it.validity().toInt() } ?: 0) + (if (id.asKnown().isPresent) 1 else 0) + (if (clientToken.asKnown().isPresent) 1 else 0) + (if (createdAt.asKnown().isPresent) 1 else 0) + (if (externalAccountId.asKnown().isPresent) 1 else 0) + (if (liveMode.asKnown().isPresent) 1 else 0) + (if (object_.asKnown().isPresent) 1 else 0) + (receivingCountries.asKnown().getOrNull()?.sumOf { it.validity().toInt() } ?: 0) + (status.asKnown().getOrNull()?.validity() ?: 0) + (if (updatedAt.asKnown().isPresent) 1 else 0)
