...
class InvoiceCreateParams private constructor(
    ...
    class InvoiceCreateRequest @JsonCreator(mode = JsonCreator.Mode.DISABLED) private constructor(
         ...
         * @throws ModernTreasuryInvalidDataException if the JSON field has an unexpected type (e.g. if the server responded with an unexpected value).
         */
        fun autoAdvance(): Optional<Boolean> = autoAdvance.getOptional("auto_advance")
         ...
         */
        @JvmSynthetic
        internal fun validity(): Int = (if (counterpartyId.asKnown().isPresent) 1 else 0) + (if (dueDate.asKnown().isPresent) 1 else 0) + (if (originatingAccountId.asKnown().isPresent) 1 else 0) + (if (autoAdvance.asKnown().isPresent) 1 else 0) + (contactDetails.asKnown().getOrNull()?.sumOf { it.validity().toInt() } ?: 0) + (counterpartyBillingAddress.asKnown().getOrNull()?.validity() ?: 0) + (counterpartyShippingAddress.asKnown().getOrNull()?.validity() ?: 0) + (currency.asKnown().getOrNull()?.validity() ?: 0) + (if (description.asKnown().isPresent) 1 else 0) + (if (fallbackPaymentMethod.asKnown().isPresent) 1 else 0) + (invoiceLineItems.asKnown().getOrNull()?.sumOf { it.validity().toInt() } ?: 0) + (invoicerAddress.asKnown().getOrNull()?.validity() ?: 0) + (if (invoicerName.asKnown().isPresent) 1 else 0) + (metadata.asKnown().getOrNull()?.validity() ?: 0) + (notificationEmailAddresses.asKnown().getOrNull()?.size ?: 0) + (if (notificationsEnabled.asKnown().isPresent) 1 else 0) + (if (paymentEffectiveDate.asKnown().isPresent) 1 else 0) + (paymentMethod.asKnown().getOrNull()?.validity() ?: 0) + (paymentType.asKnown().getOrNull()?.validity() ?: 0) + (if (receivingAccountId.asKnown().isPresent) 1 else 0) + (if (recipientEmail.asKnown().isPresent) 1 else 0) + (if (recipientName.asKnown().isPresent) 1 else 0) + (remindAfterOverdueDays.asKnown().getOrNull()?.size ?: 0) + (if (virtualAccountId.asKnown().isPresent) 1 else 0)
