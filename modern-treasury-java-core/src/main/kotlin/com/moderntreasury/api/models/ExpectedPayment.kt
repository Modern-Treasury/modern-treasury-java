...
class ExpectedPayment @JsonCreator(mode = JsonCreator.Mode.DISABLED) private constructor(
     ...
     * ```java
     * MyClass myObject = expectedPayment.reconciliationFilters().convert(MyClass.class);
     * ```
     */
     ...
     * This arbitrary value can be deserialized into a custom type using the `convert` method:
     * ```java
     * MyClass myObject = expectedPayment.reconciliationGroups().convert(MyClass.class);
     * ```
     */
     ...
     * @throws ModernTreasuryInvalidDataException if the JSON field has an unexpected type (e.g. if the server responded with an unexpected value).
     */
    fun reconciliationMethod(): Optional<ReconciliationMethod> = reconciliationMethod.getOptional("reconciliation_method")
    ...
    class Builder internal constructor() {
        ...
        /** Alias for calling [Builder.reconciliationMethod] with `reconciliationMethod.orElse(null)`. */
        fun reconciliationMethod(reconciliationMethod: Optional<ReconciliationMethod>) = reconciliationMethod(reconciliationMethod.getOrNull())
     ...
     */
    @JvmSynthetic
    internal fun validity(): Int = (if (id.asKnown().isPresent) 1 else 0) + (if (amountLowerBound.asKnown().isPresent) 1 else 0) + (if (amountReconciled.asKnown().isPresent) 1 else 0) + (amountReconciledDirection.asKnown().getOrNull()?.validity() ?: 0) + (if (amountUnreconciled.asKnown().isPresent) 1 else 0) + (amountUnreconciledDirection.asKnown().getOrNull()?.validity() ?: 0) + (if (amountUpperBound.asKnown().isPresent) 1 else 0) + (if (counterpartyId.asKnown().isPresent) 1 else 0) + (if (createdAt.asKnown().isPresent) 1 else 0) + (currency.asKnown().getOrNull()?.validity() ?: 0) + (if (dateLowerBound.asKnown().isPresent) 1 else 0) + (if (dateUpperBound.asKnown().isPresent) 1 else 0) + (if (description.asKnown().isPresent) 1 else 0) + (direction.asKnown().getOrNull()?.validity() ?: 0) + (if (externalId.asKnown().isPresent) 1 else 0) + (if (internalAccountId.asKnown().isPresent) 1 else 0) + (if (ledgerTransactionId.asKnown().isPresent) 1 else 0) + (if (liveMode.asKnown().isPresent) 1 else 0) + (metadata.asKnown().getOrNull()?.validity() ?: 0) + (if (object_.asKnown().isPresent) 1 else 0) + (reconciliationMethod.asKnown().getOrNull()?.validity() ?: 0) + (reconciliationRuleVariables.asKnown().getOrNull()?.sumOf { it.validity().toInt() } ?: 0) + (if (remittanceInformation.asKnown().isPresent) 1 else 0) + (if (statementDescriptor.asKnown().isPresent) 1 else 0) + (status.asKnown().getOrNull()?.validity() ?: 0) + (if (transactionId.asKnown().isPresent) 1 else 0) + (if (transactionLineItemId.asKnown().isPresent) 1 else 0) + (type.asKnown().getOrNull()?.validity() ?: 0) + (if (updatedAt.asKnown().isPresent) 1 else 0)
