...
private constructor(
    ...
    class Builder internal constructor() {
        ...
        internal fun from(case_: Case) = apply {
            ...
            liveMode = case_.liveMode
            object_ = case_.object_
            requestedActions = case_.requestedActions.map { it.toMutableList() }
            resolvedActions = case_.resolvedActions.map { it.toMutableList() }
            status = case_.status
            subjectId = case_.subjectId
            subjectType = case_.subjectType
    ...
    internal fun validity(): Int =
        (if (id.asKnown().isPresent) 1 else 0) +
            ...
            (requestedActions.asKnown().getOrNull()?.sumOf { it.validity().toInt() } ?: 0) +
            (resolvedActions.asKnown().getOrNull()?.sumOf { it.validity().toInt() } ?: 0) +
            (status.asKnown().getOrNull()?.validity() ?: 0) +
            (if (subjectId.asKnown().isPresent) 1 else 0) +
            (if (subjectType.asKnown().isPresent) 1 else 0) +
    ...
    private constructor(
        ...
        class Builder internal constructor() {
            ...
            fun field(field: Optional<Field>) = field(field.getOrNull())
        ...
        internal fun validity(): Int =
            (if (id.asKnown().isPresent) 1 else 0) +
                ...
                (if (liveMode.asKnown().isPresent) 1 else 0) +
                (if (object_.asKnown().isPresent) 1 else 0) +
                (reasons.asKnown().getOrNull()?.size ?: 0) +
                (if (updatedAt.asKnown().isPresent) 1 else 0)
        ...
        class Category @JsonCreator private constructor(private val value: JsonField<String>) :
            ...
            companion object {
                ...
                @JvmField val ONBOARDING_DATABASE_FAILURE = of("onboarding_database_failure")

                @JvmField
                val ONBOARDING_PROOF_OF_ADDRESS_FAILURE = of("onboarding_proof_of_address_failure")

                @JvmField val ONBOARDING_SSN_CHECK_FAILURE = of("onboarding_ssn_check_failure")

                @JvmField val ONBOARDING_TIN_CHECK_FAILURE = of("onboarding_tin_check_failure")
        ...
        class Field @JsonCreator private constructor(private val value: JsonField<String>) : Enum {
            ...
            companion object {
                ...
                @JvmField val LEGAL_ENTITY_DETAILS = of("legal_entity_details")
