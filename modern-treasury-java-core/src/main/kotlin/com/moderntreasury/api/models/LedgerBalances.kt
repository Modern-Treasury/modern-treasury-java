...
class LedgerBalances @JsonCreator(mode = JsonCreator.Mode.DISABLED) private constructor(
     ...
     */
    @JvmSynthetic
    internal fun validity(): Int = (availableBalance.asKnown().getOrNull()?.validity() ?: 0) + (pendingBalance.asKnown().getOrNull()?.validity() ?: 0) + (postedBalance.asKnown().getOrNull()?.validity() ?: 0)
