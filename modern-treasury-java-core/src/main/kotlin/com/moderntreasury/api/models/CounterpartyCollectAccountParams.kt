...
class CounterpartyCollectAccountParams private constructor(
    ...
    class CounterpartyCollectAccountRequest @JsonCreator(mode = JsonCreator.Mode.DISABLED) private constructor(
         ...
         */
        @JvmSynthetic
        internal fun validity(): Int = (direction.asKnown().getOrNull()?.validity() ?: 0) + (if (customRedirect.asKnown().isPresent) 1 else 0) + (fields.asKnown().getOrNull()?.sumOf { it.validity().toInt() } ?: 0) + (if (sendEmail.asKnown().isPresent) 1 else 0)
