...
class LedgerBalance @JsonCreator(mode = JsonCreator.Mode.DISABLED) private constructor(
     ...
     */
    @JvmSynthetic
    internal fun validity(): Int = (if (amount.asKnown().isPresent) 1 else 0) + (if (amountString.asKnown().isPresent) 1 else 0) + (if (credits.asKnown().isPresent) 1 else 0) + (if (creditsString.asKnown().isPresent) 1 else 0) + (if (currency.asKnown().isPresent) 1 else 0) + (if (currencyExponent.asKnown().isPresent) 1 else 0) + (if (debits.asKnown().isPresent) 1 else 0) + (if (debitsString.asKnown().isPresent) 1 else 0)
